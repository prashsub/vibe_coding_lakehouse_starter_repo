...
Usage:
    python .cursor/skills/admin/skill-freshness-audit/scripts/scan_skill_freshness.py
...
def main():
    # Find project root (look for .cursor/skills directory)
    cwd = Path.cwd()
    root = cwd

    # Walk up to find .cursor/skills
    for _ in range(10):
        if (root / ".cursor" / "skills").exists():
            break
        root = root.parent
    else:
        print("ERROR: Could not find .cursor/skills directory.", file=sys.stderr)
