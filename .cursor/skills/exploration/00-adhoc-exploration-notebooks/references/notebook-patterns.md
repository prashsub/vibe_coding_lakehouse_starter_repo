...
Complete code patterns for both `.py` and `.ipynb` formats, helper function implementations, widget fallback patterns, Spark session initialization, and Asset Bundle integration.
...
def check_data_quality(catalog_name: str, schema_name: str, table_name: str) -> None:
    ...
    # Duplicate check (if primary key exists)
    print("\n🔍 Duplicate Check:")
    # Note: This is a simplified check - adjust based on your primary key columns
...
list_tables(bronze_catalog, bronze_schema)
explore_table(bronze_catalog, bronze_schema, "bronze_orders")
check_data_quality(bronze_catalog, bronze_schema, "bronze_orders")
```
...
list_tables(silver_catalog, silver_schema)
explore_table(silver_catalog, silver_schema, "silver_orders")
check_data_quality(silver_catalog, silver_schema, "silver_orders")
```
...
list_tables(gold_catalog, gold_schema)
explore_table(gold_catalog, gold_schema, "dim_store")
check_data_quality(gold_catalog, gold_schema, "dim_store")
show_table_properties(gold_catalog, gold_schema, "dim_store")
```
...
```python
def analyze_sales_trends(catalog: str, schema: str, fact_table: str, days: int = 30):
    """Analyze sales trends for the last N days."""
    from pyspark.sql.functions import sum, date_sub, current_date
    
    df = spark.table(f"{catalog}.{schema}.{fact_table}")
...
```python
def track_schema_changes(catalog: str, schema: str, table_name: str):
