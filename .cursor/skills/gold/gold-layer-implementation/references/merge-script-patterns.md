...
Complete pattern for slowly changing dimensions with historical tracking:
...
Complete pattern for pre-aggregated fact tables:
...
def merge_fact_sales_daily(spark: SparkSession, catalog: str, silver_schema: str, gold_schema: str):
    ...
    distinct_grain_count = daily_sales.select(
...
**Key:** Dimensions are merged BEFORE facts to ensure FK references are populated.
