...
from pyspark.sql import types as T  # ⚠️ MANDATORY for output_data_type
```
...
- **⚠️ `output_data_type` MUST be `T.StructField().json()` format, NOT string**
...
**⚠️ CRITICAL:** Snapshot monitors need baseline table for drift metrics

| Monitor Mode | Consecutive Windows? | Drift Without Baseline? |
|--------------|---------------------|------------------------|
| **TimeSeries** | ✅ Yes | ✅ Yes (consecutive comparison) |
| **Snapshot** | ❌ No | ❌ NO (must provide baseline_table) |
...
create_monitor_with_custom_metrics(
    ...
    baseline_table=f"{catalog}.{schema}.{table}_baseline",  # ✅ Required for drift
...
MonitorMetric(
    ...
    input_columns=[":table"],  # ✅ Table-level business KPI
...
MonitorMetric(
    ...
    input_columns=[":table"],  # ✅ Must match AGGREGATE
